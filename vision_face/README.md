# VISION Face — Flutter frontend

An animated, expressive frontend for your VISION assistant: a living orb with
eyes and a mouth that react to VISION's voice, distinct expressions for every
...
backend through a small WebSocket bridge — nothing about how VISION works is
changed.

```
┌──────────────┐   UDP 5016    ┌────────────────┐   WebSocket 8765   ┌─────────────┐
│ dynamic_     │ ───────────▶ │ vision_bridge  │ ─────────────────▶ │  Flutter    │
│ island.py    │  state +      │ .py            │   state JSON        │  vision_    │
│ (HUD, audio) │  amplitude    │                │ ◀───────────────── │  face       │
└──────────────┘               └────────────────┘   commands          └─────────────┘
                                   │   ▲
                        UDP 5004   │   │ UDP 5006-5010
                        (text)     ▼   (media)
                                 VISION agent / media tool
```
...
- **Expressive animated face** (`lib/face/`) — breathing aura, fluid blobs,
  ...
  mouth that opens with VISION's voice amplitude, a circular speaking waveform,
...
- **Music player** (`lib/widgets/music_player_card.dart`) — shows the track
  VISION is playing with artwork, an animated equalizer, and play/pause + stop
...
- **Command bar** — type a command and it's sent to the agent (same path as the
  HUD's Ctrl+K box).
- **Demo mode** — with no backend running, the app animates a believable VISION
...
the VISION backend — all together. First run scaffolds and builds the Flutter
Linux app (a few minutes); later runs are fast. `./stop_vision.sh` stops
...
Then, with VISION running (so `dynamic_island.py` is up), start the bridge:
...
flip from **DEMO** to **LIVE** automatically. The mouth now moves with VISION's
real voice, the weather is your location, and the music card mirrors whatever
`Tools/media.py` is playing.

To start the bridge automatically, add this line to `start_vision.sh` (after the
HUD launches):
...
| `ai_level` | 0..1 | VISION voice amplitude (drives mouth + waveform) |
...
| `response` | string | last thing VISION said |
...
- **WebSocket URL** — default `ws://127.0.0.1:8765`. Change it in
  `VisionConnection(url: ...)` in `lib/main.dart` if you run the bridge
  elsewhere.
