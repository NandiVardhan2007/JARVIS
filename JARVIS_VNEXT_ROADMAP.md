# JARVIS vNext — Roadmap & Gap Analysis
...
- Added `start_voice_reenrollment` as a proper function_tool so re-enrollment
  can happen through conversation ("JARVIS, re-register my voice") instead
...
**Not started this session:** Latency profiling/tuning, a distinct "JARVIS
...
**Not started:** No current memory/CPU budget monitoring for JARVIS's own
...
in the previous turn); if you want JARVIS to manage packages, the safer
...
`jarvis_face/lib/face/` (~1,150 lines across three files) already contains a
...
- **Actually connected**: `jarvis_connection.dart` maintains a live
  WebSocket link to `jarvis_bridge.py`, with automatic graceful fallback to
...
**Also added:** a real light theme (`JarvisTheme.buildLight()`, adapted
...
probably the *better* aesthetic choice for a "JARVIS"-style AI than chasing
