...
async def get_stock_price(symbol: str) -> str:
    ...
    try:
        ...
        return f"{name} ({symbol.upper()}): ${price:.2f}" if price else f"Price not available for {symbol}"
...
async def portfolio_summary() -> str:
    ...
    try:
        with sqlite3.connect(DB_PATH) as conn:
            rows = conn.execute("SELECT symbol, asset_type, quantity FROM portfolio").fetchall()
...
@function_tool
async def add_to_portfolio(symbol: str, quantity: float, asset_type: str = "stock") -> str:
    """
...
__all__ = ["get_stock_price", "get_crypto_price", "portfolio_summary", "add_to_portfolio"]
